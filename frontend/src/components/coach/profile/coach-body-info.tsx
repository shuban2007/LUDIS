...
// Ludis — Coach Body Information Component
import { useState } from 'react';
import type { CoachProfileData } from '@/lib/types/profile';
import { Card } from '@/components/ui/card';

interface CoachBodyInfoProps {
  coach: CoachProfileData;
  onUpdateBody: (updates: { height?: number; weight?: number }) => void;
}

export function CoachBodyInfo({ coach, onUpdateBody }: CoachBodyInfoProps) {
  const currentHeight = coach.height ?? 180;
  const currentWeight = coach.weight ?? 78;
        ...
        <form onSubmit={handleSave} className="space-y-4 pt-1">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label htmlFor="coach-height-input" className="text-xs font-semibold text-foreground-secondary">
                Height (cm)
              </label>
              <input
                id="coach-height-input"
                type="number"
                value={heightInput}
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="coach-weight-input" className="text-xs font-semibold text-foreground-secondary">
                Weight (kg)
              </label>
              <input
                id="coach-weight-input"
                type="number"
                value={weightInput}
