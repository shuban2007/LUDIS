...
- `HeroSection` (`src/components/landing/hero-section.tsx`) — Client component wrapping hero copy + product preview with staggered `HeroReveal` entrance animations (eyebrow → headline → paragraph → CTAs → product preview). Features refined, bolder heading typography (`font-semibold italic` for "KNOW YOUR BODY." and `font-extrabold` for "TRAIN SMARTER.") and clean subheadline copy without unnecessary em-dash breaks ("insights so you can perform at your best, every day."). Unifies DOM rendering for both themes, presenting the custom light mode runner (`public/runner_light_mode.png`) dynamically alongside a resized and bottom-right aligned glass telemetries dashboard preview (`HeroProductPreview`) in light mode, while preserving default runner visuals in dark mode.
- `FinalCTA` (`src/components/landing/sections.tsx`) — Final landing call-to-action section featuring refined "TRAIN WITH CLARITY." heading typography (`font-medium`, `tracking-normal`/`tracking-[0.02em]`, `[word-spacing:0.12em]`, `leading-[1.1]`, `antialiased`) eliminating serif glyph collisions and providing distinct word separation.
...
- `BaselineVisual` (`src/components/landing/baseline-visual.tsx`) — Visual baseline normal range vs. current reading gauge with scroll-reveal animations and optimized vertical padding (`py-7 sm:py-9 md:py-10`).
- `MvpPillars` (`src/components/landing/mvp-pillars.tsx`) — Glassmorphism cards for the 4 MVP pillars with 80ms staggered entrance, hover card rise, text color transitions, and controlled title margin (`mb-8 sm:mb-10 md:mb-12`) eliminating legacy 96px empty gap.
- `AudienceSection` & `HowLudisThinks` (`src/components/landing/sections.tsx`) — Athlete/Coach interfaces (staggered cards) featuring mouse pointer glow, 3D card tilt, compact touch targets, optimized section padding (`py-7 sm:py-9 md:py-10`), and a static overflow-hidden crop boundary with an inset-[-24px] parallax translation wrapper to prevent card background bleeding. Includes direct authentication integrations mapped to Quick Demo logins.
- `CompetitionContext` & `ResponsibleAI` (`src/components/landing/sections.tsx`) — Event weather context & ethical decision support boundaries with minimal scroll reveals and streamlined section spacing above value marquee (`pt-7 sm:pt-9 pb-0`).
- `LandingBackToTop` (`src/components/landing/landing-back-to-top.tsx`) — Minimal, high-performance floating back-to-top button exclusively mounted on the landing page (`src/app/page.tsx`). Activates when `window.scrollY > 400px` with a 200ms entrance/exit transition (`opacity: 0/1`, `translateY(8px/0)`), positioned `fixed` in the bottom-right area with safe-area calculations (`env(safe-area-inset-bottom)`), executing smooth scroll to top (`window.scrollTo({ top: 0, behavior: 'smooth' })`) without changing URL or triggering page reloads.
- `LandingValueMarquee` (`src/components/landing/landing-value-marquee.tsx`) — Continuous right-to-left scrolling marquee banner replacing the legacy static feature strip inside `ResponsibleAI`. Features 12 core value statements with inline SVG line icons and teal dot separators. Utilizes GPU-accelerated CSS keyframe animation (`translate3d`), dual-track duplication for seamless infinite looping, subtle linear edge gradient masking (`marquee-edge-mask`), theme-aware variables, and `prefers-reduced-motion` pause behavior.
